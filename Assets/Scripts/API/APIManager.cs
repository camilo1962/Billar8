        ...
        void ResponseForAPIValidateEmailPlayer(HTTPRequest request, HTTPResponse response)
        {
            switch (request.State)
            {
                ...
                case HTTPRequestStates.Aborted:
                    ...
                    Debug.LogWarning("¡Solicitud abortada!");
                    break;

                // Connecting to the server is timed out.
                case HTTPRequestStates.ConnectionTimedOut:

                    Debug.LogError("¡Tiempo de conexión agotado!");
                    break;

                // The request didn't finished in the given time.
                case HTTPRequestStates.TimedOut:

                    Debug.LogError("Procesando la solicitud ¡Se agotó el tiempo!");
                    break;
            }
        ...
        void ResponseForAPILoginEmailPlayer(HTTPRequest request, HTTPResponse response)
        {
            switch (request.State)
            {
                ...
                case HTTPRequestStates.Error:
                    ...
                    Debug.LogError("\n¡Solicitud finalizada con error! " + (request.Exception != null ? (request.Exception.Message + "\n" + request.Exception.StackTrace) : "No Exception"));
            ...
            switch (request.State)
            {
                ...
                case HTTPRequestStates.Finished:
                    ...
                    else
                    {
                        ...
                        Debug.LogWarning(string.Format("La solicitud finalizó con éxito, pero el servidor envió un error. Código de estado: {0}-{1} Mensaje: {2}",
                                                        response.StatusCode,
                                                        response.Message,
                                                        response.DataAsText));
                    }
                ...
                case HTTPRequestStates.Error:
                    ...
                    Debug.LogError("¡Solicitud finalizada con error! " + (request.Exception != null ? (request.Exception.Message + "\n" + request.Exception.StackTrace) : "Sin excepción"));
