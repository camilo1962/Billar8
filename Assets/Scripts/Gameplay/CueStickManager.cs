    ...
    public class CueStickManager : MonoBehaviour
    {
        ...
        public RaycastHit GetHitPoint()
        {
            ...
            else
            {
               ...
               ///// RaycastHit hit;
               ///int layerMask = LayerMask.GetMask("BolaBlanca"); // Obtener la máscara de capa para la capa "BolaBlanca"
               ///if (Physics.Raycast(cueStick.localPosition, BallFinalPos, out hit, maxLength, layerMask))
               ///{
               ///    // Verifica si el raycast detectó una colisión con la bola blanca
               ///    if (centerHit.collider != null && centerHit.collider.CompareTag("CueBall"))
               ///    {
               ///        // Reproduce el sonido del taco golpeando la bola blanca
               ///        if (SoundManager.instance.cueBallCollisionClip != null)
               ///        {
               ///            print("bola blanca golpeada");
        ...
        public void DisplayGuidelines(RaycastHit hit, Vector3 cueBallHitPosition)
        {
            ...
            if (hit.collider.tag == "Ball") // solo muestra líneas reflejadas si es una bola objetivo
            {
